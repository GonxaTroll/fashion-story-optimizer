...
import FooterInfoModal, { type FooterTab } from '@/components/FooterInfoModal'
...
const INFO_LINKS: { label: string; tab: FooterTab }[] = [
...
export default function AppFooter() {
  const [openTab, setOpenTab] = useState<FooterTab | null>(null)
      ...
      <footer className="bg-[#ffd7f0] rounded-t-[2rem] py-10 px-6">
        <div className="max-w-7xl mx-auto flex flex-col items-center gap-6">
          ...
          <div className="flex flex-col items-center gap-2">
            <p className="text-xs font-bold text-[#784e6c] uppercase tracking-widest">
          ...
          {/* Info links — open modal */}
          <div className="flex flex-wrap justify-center gap-8">
            {INFO_LINKS.map(({ label, tab }) => (
              <button
                key={tab}
                type="button"
                onClick={() => setOpenTab(tab)}
                className="text-sm font-medium text-[#B02E7A] hover:underline underline-offset-4
                           decoration-2 transition-all duration-150 cursor-pointer
                           focus:outline-none focus:ring-1 focus:ring-[#B02E7A] rounded"
              >
                {label}
              </button>
            ))}
          ...
          <p className="text-xs font-bold text-[#966988] uppercase tracking-widest text-center">
            © 2026 FashStOpt · Stay Playful. · Not affiliated with Storm8 Studios.
          </p>
        </div>
      </footer>
    </>
